...
func (r *translationRoutes) doTranslate(ctx *fiber.Ctx) error {
	...
	if err := ctx.BodyParser(&request); err != nil {
		r.l.Error("http - v1 - doTranslate - body parse error", zap.Error(err))

		return errorResponse(ctx, http.StatusBadRequest, "invalid request body")
	}

	if err := r.v.Struct(request); err != nil {
		r.l.Error("http - v1 - doTranslate - validation error", zap.Error(err))
	...
	if err != nil {
		r.l.Error("http - v1 - doTranslate - translation error", zap.Error(err))
